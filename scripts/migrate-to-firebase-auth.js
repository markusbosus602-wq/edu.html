...
async function main() {
  ...
  if (!snap.exists()) {
    console.log('Стару гілку users/{нік} не знайдено — міграція не потрібна.');
    return;
  }

  const legacyUsers = snap.val();
  const nicks = Object.keys(legacyUsers);
  console.log(`Знайдено ${nicks.length} гравців у старій схемі.\n`);

  // Робимо локальний бекап перед будь-якими змінами
  if (!DRY_RUN && !CLEANUP) {
  ...
  for (const nick of nicks) {
    ...
    if (CLEANUP) continue; // на етапі cleanup нижче не потрібно
  ...
  if (CLEANUP) {
    console.log('\n🧹 Видаляю стару гілку users/{нік} з паролями відкритим текстом...');
    await db.ref(OLD_USERS_PATH).remove();
    console.log('✅ Стару гілку видалено. Переконайтесь, що резервна копія (legacy-users-backup.json) збережена в надійному місці окремо від репозиторію.');
    return;
  }
  ...
  if (!DRY_RUN) {
    ...
    console.log('щоб остаточно видалити стару гілку з паролями.');
  }
}
